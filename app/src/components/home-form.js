...
class HomeForm extends Component{
    ...
    _onSubmit(event){
        ...
        this._formValidation(['from', 'to', 'files'], (isValid) => {
            ...
            if(isValid){
                // the form is validate and ready to submit
                            ...
                            <div className= {classNames('app-file-select-zone', {'error': _.get(errors, 'files') })}>
                                <label htmlFor={'input-file'}>
                                    <input onChange={this._onFileAdded} id={'input-file'} type="file" multiple={true} />
                                    {
                                        ...
                                        :   <span>
                                                <span className={'app-upload-icon'}> <i className={'icon-picture-o'} />  </span> <span className={'app-upload-description'}> Drag and drop your files here. </span>
                                            </span>
                                    }
                                </label>
                            </div>
                    ...
                    <div className={'app-card-content'}>
                        <div className={'app-card-content-inner'}>
                            <div className={classNames('app-form-item', {'error': _.get(errors, 'to')})}>
                                ...
                                <input onChange={this._onTextChange} value={form.to} name={'to'} placeholder={_.get(errors, 'to') ? _.get(errors, 'to') : 'Receiver\'s Email Address'} type={'text'} id={'id'} />
                            </div>

                            <div className={classNames('app-form-item', {'error': _.get(errors, 'from')})}>
                                <label htmlFor={'from'}>From </label>
                                <input onChange={this._onTextChange} value={form.from} name={'from'} placeholder={_.get(errors, 'from') ? _.get(errors, 'from') : 'Sender\'s Email Address'} type={'text'} id={'from'} />
                            </div>

                            <div className={'app-form-item'}>
                                <label htmlFor={'message'}>Message </label>
                                <textarea onChange={this._onTextChange} value={form.message} placeholder={'Add A Note (Optional)'} id={'message'} name={'message'} />
                            </div>
